...
impl ResourceRegistry {
    ...
    pub async fn register<T: Resource>() -> anyhow::Result<()> {
        let value = T::init().await?;
        Self::register_with(value);
        Ok(())
    }
    ...
    /// Must only be called during service startup, before any call to
    /// [`Self::get`] for the same type.
    pub fn register_with<T: Any + Send + Sync>(value: T) {
        REGISTRY.0.insert(TypeId::of::<T>(), Arc::new(value));
    }
...
mod tests {
    ...
    fn resources_test<T: Debug + Clone + PartialEq + Eq + Send + Sync + 'static>(v: T) {
        ResourceRegistry::register_with(v.clone());
