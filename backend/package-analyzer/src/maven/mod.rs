...
pub use types::MavenPackage;
use types::parse_dependency_line;
...
mod tests {
    ...
    const TEST_MANIFEST: &[u8] = include_bytes!("test.dependencies.txt");

    #[test]
    fn parse_manifest_test() {
        let mut packages: Vec<_> = MavenPackage::parse_manifest(TEST_MANIFEST)
