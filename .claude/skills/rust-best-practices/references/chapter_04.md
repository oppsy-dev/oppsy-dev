...
```
* If your `Result` (or `Option`) needs error recovery in case of `Result::Err` and access to the original input, use `if let … else`:
```rust
if let Ok(json) = serde_json::from_str(&input) {
    // use json
} else {
    return Err(do_something_with_input(&input));
}
```
